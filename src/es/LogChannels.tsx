    ...
    <div className="LogChannels">
      {logChannels.map((logChannel, index) => (
        <div className="log-channel">
          ...
          <button onClick={() => deleteLogChannel(index)}>Eliminar</button>
        </div>
      ))}
      <button onClick={() => addLogChannel()}>Añadir</button>
      <button onClick={() => addReverseLogChannel()}>Añadir "todo lo demás"</button>
    </div>
  );
}
